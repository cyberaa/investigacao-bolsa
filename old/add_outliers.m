...
%
%%%%
function X=add_outliers(X, oversampling_samples, num_samples, num_outliers, min_outlier_thresh, max_outlier_thresh)

    %%%%
    %% Select which indexes we are going to turn into outliers
    %%%%
    outlier_indexes = zeros(1,num_samples);
    for i=1:num_outliers
        while 1 == 1
            ind = randi([1, num_samples]);
    ...
    for i=1:num_outliers
        ...
        X(1+ind*oversampling_samples) = X(1+ind*oversampling_samples) + sign * ((max_outlier_thresh-min_outlier_thresh)*rand()+min_outlier_thresh);
    end
end